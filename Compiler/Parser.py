    ...
    def __call__(self, w):
        ...
        while True:
            ...
            if action == ShiftReduceParser.SHIFT:
                ...
                _ ,iD = self.goto.get((stack[-1],left))
                stack.append(iD)
                output.append(prod)
            ...
            for item in node.state:
                ...
                if next_symbol is None: 
                    for l in item.lookaheads:
                        ...
                        LR1Parser._register(self.action, (idx,l),  (ShiftReduceParser.REDUCE, k))
                        
                elif next_symbol.IsNonTerminal:
                    transit = node.transitions.get(next_symbol.Name)
                    if transit:
                        LR1Parser._register(self.goto, (idx,next_symbol),  (None, transit[0].idx))

                else: 
                    
                    transit = node.transitions.get(next_symbol.Name)
                    if transit:
                        LR1Parser._register(self.action, (idx,next_symbol), (ShiftReduceParser.SHIFT, transit[0].idx))
